...
from .steering import get_prompt_config
...
class MedGemmaEngine:
    ...
    def _call_server(self, messages: List[Dict[str, str]]) -> Optional[str]:
        """
        Call local llama.cpp server using the MedGemmaAgent middleware.
        This ensures GBNF Grammar, Dual Endpoints, and Thought Stripping are enforced.
        """
        try:
            from medgemma_client import MedGemmaAgent
            
            # Initialize the agent (defaults to localhost:8080)
            agent = MedGemmaAgent()
            
            # Parse LangGraph's message array into distinct prompts
            system_prompt = next((m["content"] for m in messages if m["role"] == "system"), "")
            
            # Safely concatenate user history if there are multiple turns
            user_prompt = "\n".join([m["content"] for m in messages if m["role"] != "system"])
            
            # Safe truncation: ~4 chars per token.
            # 1500 chars (sys) + 6000 chars (user) = ~1875 tokens.
            # This leaves >2000 tokens for the model's output on an 8GB machine.
            MAX_SYS_CHARS = 1500
            MAX_USER_CHARS = 6000
            
            if len(system_prompt) > MAX_SYS_CHARS:
                system_prompt = system_prompt[:MAX_SYS_CHARS] + "... [TRUNCATED]"
                
            if len(user_prompt) > MAX_USER_CHARS:
                user_prompt = user_prompt[:MAX_USER_CHARS] + "... [TRUNCATED]"
            
            # Execute the call through the middleware
            ...
            logger.error(f"MedGemmaAgent call failed: {e}")
            return None
    ...
    def _call_model(self, messages: List[Dict[str, str]]) -> Optional[str]:
        ...
        try:
            ...
            # MedGemmaAgent already handles thought stripping, so return directly
            return result
        except Exception as e:
    ...
    def generate_night_report(
        ...
        Args:
            patient_context: Patient identification and context
            night_summary: Summary of night surveillance period
        ...
        # Get dynamic prompt configuration from steering matrix
        config = get_prompt_config(patient_profile, "night")
    ...
    def generate_day_report(
        ...
        Args:
            patient_context: Patient identification and context
            night_context: Summary of night surveillance
            consultation_data: Day consultation data
            specialty: Medical specialty (legacy parameter, kept for compatibility)
            patient_profile: Patient clinical profile ("cardiac", "respiratory", "general")
        """
        if not self.is_loaded:
        ...
        # Get dynamic prompt configuration from steering matrix
        # Use patient_profile for steering, specialty is legacy parameter
        config = get_prompt_config(patient_profile, "day")
    ...
    def _build_night_prompt(
        ...
        # Only use the most recent 20 events to stay within token limits
